...
export const getServicesRequest = async (filters?: {
  ...
  try {
    ...
    const response = await api.get(`/dashboard/ordenes-servicio?${params.toString()}`);
    return response.data.data;
  } catch (error: any) {
...
export const getMyServicesRequest = async (filters?: {
  ...
  try {
    ...
    const response = await api.get(`/dashboard/mis-servicios?${params.toString()}`);
    return response.data.data;
  } catch (error: any) {
    console.error('Error obteniendo mis servicios:', error);
    throw new Error(error.response?.data?.error || 'Error al obtener mis servicios');
  }
};