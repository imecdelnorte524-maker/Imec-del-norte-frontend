...
type RoleKey = 'ADMINISTRADOR' | 'SUPERVISOR' | 'SECRETARIA' | 'SG-SST' | 'TECNICO';

// Mapeo de nombres de roles (para manejar diferentes formatos)
export const ROLE_NAMES: Record<RoleKey, readonly string[]> = {
  ADMINISTRADOR: ['ADMINISTRADOR', 'Administrador', 'administrador', '1'],
  SUPERVISOR: ['SUPERVISOR', 'Supervisor', 'supervisor', '2'],
  SECRETARIA: ['SECRETARIA', 'Secretaria', 'secretaria', '3'],
  'SG-SST': ['SG-SST', 'SGSST', 'sgsst', '4'],
  TECNICO: ['TECNICO', 'Técnico', 'técnico', 'tecnico', '5']
} as const;
...
export const hasFullAccess = (roleName?: string): boolean => {
  ...
  if (typeof normalizedRole === 'string' && Object.keys(ROLE_NAMES).includes(normalizedRole)) {
...
export const getUserAccessLevel = (roleName?: string): 'full' | 'own' | 'none' => {
...
export const checkRoleAccess = (roleName?: string) => {
  ...
  const isFullAccess = FULL_ACCESS_ROLES.some(role => 
    role.toUpperCase() === roleUpper
  );
  
  // Verificar acceso propio
  const isOwnAccess = OWN_ACCESS_ROLES.some(role => 
    role.toUpperCase() === roleUpper
  );
  ...
  return { level, canView, canCreate, isSST: roleUpper.includes('SGSST') || roleUpper.includes('SG-SST') };
};